<template>
  <div class="p-picker wheel-scroll" @click="cancelClick">
    <div class="bg"></div>
    <transition name="slide-fade">
      <div class="piciker-cont" v-show="visible">
        <div class="btns">
  ...
  methods: {
    init () {
      ...
      setTimeout(() => {
        ...
        for (let m = 0; m < sum; m++) {
          ...
          this['datePicker' + m].on('scrollStart', () => {
            this.canClick = false
          })
    ...
    sureClick () {
      ...
      for (let m = 0; m < this.hierarchy.length; m++) {
        this.texts.push(this.dateData[m][this.hierarchy[m]])
      }
      if (!this.texts[this.texts.length - 1]) {
        this.texts[this.texts.length - 1] = this.dateData[this.dateData.length - 1][this.dateData[this.dateData.length - 1].length - 1]
      }
      console.log(this.texts)
      // this.hierarchy,
      this.$emit('select', this.texts)
    },
    cancelClick () {
      this.visible = false
    ...
    updateData (pos, m) {
      ...
      const es = this.endDate.split('/')
      let delayer = 0
      this.hierarchy[m] = parseInt(Math.abs(pos.y / 30))
      if (m === 0 || m === 1) {
        if (m === 0) {
          delayer += 20
          if (pos.y - 0 === 0) {
            this.$set(this.dateData, 1, this.setData(ss[1] - 0, 12))
          } else if (parseInt(Math.abs(pos.y / 30)) === es[0] - ss[0]) {
